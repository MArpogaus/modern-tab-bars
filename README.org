...
The id the package is installed by is =modern-tabs=, the name of the
repository; the prefix of every symbol is =modern-tab-=, after the
main file that prefix comes from, =modern-tab.el=.  Both modes are
autoloaded:
