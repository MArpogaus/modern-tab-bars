...
font.  A graphic frame without the font falls back to a plain
character.  A terminal is only asked whether it can encode the glyph,
not whether it has the font, so a UTF-8 terminal without a nerd font
shows a box: name plain strings in the icon options there.
