...
The buttons of the two rows are one family.  Every one of them leads
with a codicon, so the new button, the close button of both rows, the
menu button and the mark on the selected tab are drawn at one weight
and one visual size.

Icons come from [[https://github.com/rainstormstudio/nerd-icons.el][nerd-icons]] where it is installed, and a plain =?= stands
where an icon option names a glyph it cannot supply.  A graphic frame
...
That refusal is a guess about a terminal Emacs cannot ask.  A terminal
picks the font and draws the characters itself, and =char-displayable-p=
answers for the coding system alone, so a glyph the font lacks arrives
as a box rather than being refused.  Where your terminal does carry the
icon font, say so and it draws what a graphic frame draws:

#+begin_src emacs-lisp
  (setopt modern-tab-terminal-glyphs t)
#+end_src

The coding system is still asked, so a candidate the terminal cannot
encode is still refused, and setting it back to nil brings the plain
candidates straight back.
...
An icon is a string shown as it is, a plist naming a nerd-icons glyph,
or a function of no arguments that returns a string:
...
The icon function is =modern-tab-line-file-icon= by default, which asks
nerd-icons about the name of the buffer as though it were a file name.
