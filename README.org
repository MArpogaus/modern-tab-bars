...
Icons come from [[https://github.com/rainstormstudio/nerd-icons.el][nerd-icons]] where it is installed and the frame has the
font.  Each frame gets the glyph that frame can draw, so one daemon
serves a graphic frame and a terminal frame at once.  A graphic frame
without the font falls back to a plain character.  A terminal has no
font to ask, so a nerd glyph falls back there too.  A symbol a UTF-8
terminal can encode but its font has no glyph for still shows a box:
name plain strings in the icon options there.
...
The package is called =modern-tab=, after its main file, and both modes
are autoloaded from it:

#+begin_src emacs-lisp
  (use-package modern-tab
    :vc (:url "https://github.com/MArpogaus/modern-tab-bars" :rev :newest)
    :hook ((after-init . modern-tab-bar-mode)
           (after-init . modern-tab-line-mode)))
#+end_src

Drop either hook to run one mode alone.
...
=global-tab-line-mode= on.  Turning it off turns =global-tab-line-mode=
off again, whether or not you had it on before.  A window showing one
...
separator and its automatic width, the tab line's two buttons — and
gives every one of them back when it is turned off.
